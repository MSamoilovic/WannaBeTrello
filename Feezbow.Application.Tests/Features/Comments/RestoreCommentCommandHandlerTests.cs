...
namespace Feezbow.Application.Tests.Features.Comments;

public class RestoreCommentCommandHandlerTests
{
    [Fact]
    public async Task Handle_WhenUserIsAuthenticatedAndOwnsComment_ShouldRestoreCommentAndReturnSuccessResponse()
    {
        // Arrange
        var userId = 123L;
        var commentId = 456L;
        var command = new RestoreCommentCommand(commentId);
        ...
        commentServiceMock
            .Setup(s => s.RestoreCommentAsync(commentId, userId, It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);

        var cacheServiceMock = new Mock<ICacheService>();

        var handler = new RestoreCommentCommandHandler(commentServiceMock.Object, currentUserServiceMock.Object, cacheServiceMock.Object);
        ...
        Assert.Equal("Comment restored successfully", response.Result.Message);

        commentServiceMock.Verify(s => s.RestoreCommentAsync(commentId, userId, It.IsAny<CancellationToken>()), Times.Once);
    ...
    {
        // Arrange
        var command = new RestoreCommentCommand(456L);
        ...
        commentServiceMock.Verify(s => s.RestoreCommentAsync(It.IsAny<long>(), It.IsAny<long>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task Handle_WhenCommentDoesNotExist_ShouldPropagateNotFoundException()
    {
        ...
        var command = new RestoreCommentCommand(nonExistentCommentId);
        ...
        var commentServiceMock = new Mock<ICommentService>();
        commentServiceMock
            .Setup(s => s.RestoreCommentAsync(nonExistentCommentId, userId, It.IsAny<CancellationToken>()))
        ...
        commentServiceMock.Verify(s => s.RestoreCommentAsync(nonExistentCommentId, userId, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Handle_WhenUserDoesNotOwnComment_ShouldPropagateAccessDeniedException()
    {
        ...
        commentServiceMock
            .Setup(s => s.RestoreCommentAsync(commentId, userId, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new AccessDeniedException("Samo autor komentara može vratiti komentar."));
        ...
        Assert.Equal("Samo autor komentara može vratiti komentar.", exception.Message);
        commentServiceMock.Verify(s => s.RestoreCommentAsync(commentId, userId, It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task Handle_WhenCommentIsNotDeleted_ShouldPropagateBusinessRuleValidationException()
    {
        // Arrange
        var userId = 123L;
        var commentId = 456L;
        ...
        commentServiceMock
            .Setup(s => s.RestoreCommentAsync(commentId, userId, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new BusinessRuleValidationException("Komentar nije obrisan."));
        ...
        Assert.Equal("Komentar nije obrisan.", exception.Message);
        commentServiceMock.Verify(s => s.RestoreCommentAsync(commentId, userId, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Handle_WithValidCommand_ShouldPassCorrectParametersToService()
    {
        ...
        commentServiceMock
            .Setup(s => s.RestoreCommentAsync(commentId, userId, cancellationToken))
            .Returns(Task.CompletedTask);
        ...
        commentServiceMock.Verify(
            s => s.RestoreCommentAsync(commentId, userId, cancellationToken),
            Times.Once);
    }
    ...
    public async Task Handle_MultipleCallsWithSameComment_ShouldCallServiceEachTime()
    {
        ...
        // Assert
        commentServiceMock.Verify(s => s.RestoreCommentAsync(commentId, userId, It.IsAny<CancellationToken>()), Times.Exactly(2));
    }
    ...
    public async Task Handle_WithDifferentComments_ShouldCallServiceWithCorrectIds()
    {
        ...
        var command1 = new RestoreCommentCommand(commentId1);
        var command2 = new RestoreCommentCommand(commentId2);
        ...
        commentServiceMock
            .Setup(s => s.RestoreCommentAsync(It.IsAny<long>(), userId, It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        ...
        commentServiceMock.Verify(s => s.RestoreCommentAsync(commentId1, userId, It.IsAny<CancellationToken>()), Times.Once);
        commentServiceMock.Verify(s => s.RestoreCommentAsync(commentId2, userId, It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task Handle_WhenServiceThrowsGeneralException_ShouldPropagateException()
    {
        // Arrange
        var userId = 123L;
        var commentId = 456L;
        ...
        Assert.Equal("Some unexpected error", exception.Message);
        commentServiceMock.Verify(s => s.RestoreCommentAsync(commentId, userId, It.IsAny<CancellationToken>()), Times.Once);
    }
}
